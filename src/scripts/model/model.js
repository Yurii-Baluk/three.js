...
export class Model extends EventDispatcher {
    ...
    newDesign(floorplan, items, testClick) {
        this.__roomItems = [];
        ...
        for (let i = 0; i < items.length; i++) {
           ...
           // item.__visible = false;
            ...
            //this.clearItems();
            if(item.__metadata.itemName == testClick){
                console.log("fggh")
                this.__roomItems.push(item);
            }
        }
    ...
    deleteItem(item){
        this.dispatchEvent({ type: EVENT_ITEM_SELECTED});
        for(let i = 0; i < this.__roomItems.length; i++){
            let itemMetaData = this.__roomItems[i];
            let itemType = itemMetaData.itemType;
            let item = new(Factory.getClass(itemType))(itemMetaData, this, itemMetaData.id);
            document.addEventListener("EVENT_ITEM_SELECTED", (evt) => {
                console.log("kkk");
            })
            console.log(item);
        }
     ...
     */
    remove(roomItem, keepInList, item) {
